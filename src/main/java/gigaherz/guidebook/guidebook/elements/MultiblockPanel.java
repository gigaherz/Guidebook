...
public class MultiblockPanel implements IHoverPageElement, IClickablePageElement, ITickable
{
    ...
    private static final float COLLAPSED_SCALE = 1.2f;

    private float getCollapsedScale()
    {
        return MathHelper.clamp(COLLAPSED_SCALE - 1f, 0f, 8f);
    }
    ...
    private void renderStructure(MultiblockStructure structure, IBookGraphics nav, int left, int top)
    {
        ...
        float expandLevelGap = 1f;
        float expandScale = 0.65f;
    ...
    public void parseChildren(NodeList childNodes)
    {
        for (int i = 0; i < childNodes.getLength(); ++i)
        {
            ...
            for (ParsableMultiblockComponent.Parser parser : ParsableMultiblockComponent.Parser.registry)
            {
                if (componentNode.getNodeName().equals(parser.getBaseNodeName()))
                {
                    for (MultiblockStructure structure : structures)
                    {
                        Vec3i blockPos = null;
                        Node positionNode = componentNode.getAttributes().getNamedItem("position");
                        if (positionNode != null)
                        {
                            blockPos = ParseUtils.parseVec3i(positionNode.getTextContent());
                        }
                        if (blockPos == null)
                        { // If the position is still null (if it never was parsed or parsing failed), initialize it to default
                            blockPos = new Vec3i(0, 0, 0);
                        }
                        structure.addComponent(parser.parse(componentNode), blockPos.getX(), blockPos.getY(), blockPos.getZ());
                    }
                }
            }
        }
